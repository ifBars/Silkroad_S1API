...
//{
//    public class BlackmarketBuyer : S1API.Entities.NPC
//    {
...
//                ? Path.Combine(MelonEnvironment.ModsDirectory, "Empire", "fallback-dealer.png")
...
//        {
//            MelonLogger.Msg($"Configuring prefab for dealer at index: {dealerDataIndex}");
//            MelonLogger.Msg($"Current Buyers in Contacts: count -> {Contacts.Buyers.Count}");
//			foreach (var kvp in Contacts.Buyers)
//            {
//                MelonLogger.Msg($"  Existing Buyer: {kvp.Key}");
//			}

//			Dealer currentDealer = GetDealer();

//			string dealerId = currentDealer.Name.Trim().ToLower().Replace(" ", "_");
//			string dealerFirstName = currentDealer.Name.Trim().Split(' ')[0];
//			string dealerLastName = currentDealer.Name.Trim().Contains(' ') ? currentDealer.Name.Substring(currentDealer.Name.IndexOf(' ') + 1) : "";

//            MelonLogger.Msg($"Attempting prefab configure for dealer: Index: {dealerDataIndex} - {dealerId} - {dealerFirstName} {dealerLastName} || Sprite location: {npcSprite?.name ?? "Cannot read sprite location"}");

//            builder.WithIdentity(dealerId, dealerFirstName, dealerLastName).WithIcon(npcSprite);
...
//        {
//            MelonLogger.Msg($"BlackmarketBuyer {DealerName} OnlLoaded.");
//            base.OnLoaded();
//            if (dealerDataIndex < JSONDeserializer.dealerData.Dealers.Count)
//            {
